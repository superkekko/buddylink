...
class authentication extends controller {
	...
	function login($f3) {
		...
		if(hash_equals($session_csrf, $page_token) && !empty($user_check) && $user_check['user_id'] === $username && $this->encriptDecript($user_check['password'], 'd') === $password){
	...
	function checklogged($f3){
		...
		if (!empty($user_present) && !empty($session_token) && !empty($session_username) && !empty($user_data['token_expire']) && strtotime($user_data['token_expire']) >= strtotime(date('Y-m-d H:i:s'))) {
			...
			$f3->set('active_user', array('user_id' => $user_data['user_id'], 'bearer' => $user_data['bearer']));
			return true;
		}else{
			return false;
		}
